...
//geometry_msgs::Pose desired_part_position(const geometry_msgs::Pose &msg,const int &x) {
    geometry_msgs::Pose desired_part_position(const tf::TransformListener &listener, const int &x) {
        ...
        for (osrf_gear::Order order_ : received_orders_) {
            for (osrf_gear::Kit kit_ : order_.kits) {
                for (osrf_gear::KitObject object_ : kit_.objects) {
                    // pose_pair_info.first =assign_pose(bin[num][1],bin[num][2],bin[num][3]);
                    // pose_pair_info.first =assign_pose(0,0,0);
                    //pose_pair_info.second = desired_part_position(object_.pose,num);
                    //pick_place_list.push_back(pose_pair_info);
                    //ROS_INFO_STREAM("The objects are" << kit_<<"\n" );
...
//ROS_INFO_STREAM("First x position is  " << pick_place_list[1].second.position.z);
    ...
    void poseCallback(geometry_msgs::Pose& msg, const int& x) {
        static tf::TransformBroadcaster br;
        tf::Transform transform;
        transform.setOrigin( tf::Vector3(msg.position.x, msg.position.y, msg.position.z));;
        tf::Quaternion q;
...
int main(int argc, char ** argv) {
    ...
    //Movesrv.request.mode = 2;
    //client.call(Movesrv);
    std::vector<std::vector<float> > bin = {{ -0.2, 0.33, 0.745}, { -0.4, 0.13, 0.745}, { -0.1, -0.335, 0.745}, { -0.233, -0.468, 0.745}, { -0.36, -0.335, 0.745}, {0.4, 3.15, 0.78}, { -0.4, 0.33, 0.745}, { -0.2, 0.13, 0.743}, { -0.366667, -0.468333, 0.741}, { -0.499998, -0.601719, 0.741}, { -0.1, -0.6, 0.745}, { -0.07, 0.13, 0.745}};
    onceOnly = true;
    ...
    while (ros::ok()) {
        if (!comp_class.order_pose_list.empty()) {
            ...
            for (int i = 0; i < comp_class.order_pose_list.size(); i++) {
                comp_class.poseCallback(comp_class.order_pose_list[i], i);

                geometry_msgs::Pose world_pose = comp_class.desired_part_position(listener, i);
                //  if (world_pose.position.x != 0)
                world_pose_list.push_back(world_pose);

            }
            //  comp_class.poseCallback(comp_class.order_pose_list[1],1);

            //  geometry_msgs::Pose world_pose0 = comp_class.desired_part_position(listener,1);
            ...
            if (world_pose_list[0].position.x != 0 && onceOnly) {
                for (int x = 0; x < 12; x++) {
                    if (world_pose_list[0].position.x != 0) {
                        ...
                        Movesrv.request.pose.position.z = bin[x][2];      // comp_class.pick_place_list[0].second.position.z;
                        Movesrv.request.mode = 1;
                        client.call(Movesrv);
                        if (x < 6) {
                            //ros::spinOnce();
                            ...
                            Movesrv.request.pose.position.z = 0.8; // comp_class.pick_place_list[0].second.position.z;
                            if (x == 6 || x == 7)
                                Movesrv.request.pose.orientation.z = -0.788+3.14;
                            else if (x == 11)
                                Movesrv.request.pose.orientation.z = -0.503 + 3.14;
                            else
                                Movesrv.request.pose.orientation.z = -0.0+3.14;
                ...
                onceOnly = false;
                Movesrv.request.pose.position.x = -0.3; // comp_class.pick_place_list[0].second.position.x;
                Movesrv.request.pose.position.y = 0;      // comp_class.pick_place_list[0].second.position.y;
                Movesrv.request.pose.position.z = 0.9;      // comp_class.pick_place_list[0].second.position.z;
                Movesrv.request.mode = 1;
                client.call(Movesrv);
                ros::spinOnce();
            }
    ...
    //  ros::spin();
// if(!object_list.pick_place_list.empty())
//ROS_INFO_STREAM("In mian file there are  " << comp_class.object_list.size()<<" objects\n" );
//if(!comp_class.pick_place_list.empty())
//comp_class.action(node);
